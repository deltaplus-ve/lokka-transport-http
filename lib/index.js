/* global fetch */
import LokkaTransport from 'deltaplus-lokka-context/transport';
...
export class Transport extends LokkaTransport {
  ...
  _buildOptions(payload, context) {
    const options = {
      method: 'post',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
        'Guesthub-Context': context
      },
  ...
  send(query, variables, operationName, context) {
    const payload = {query, variables, operationName};
    const options = this._buildOptions(payload, context);
